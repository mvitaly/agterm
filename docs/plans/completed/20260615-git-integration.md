...
- [x] add observed `var gitStatus: GitStatus?` to `Session` (defaults nil; `@MainActor`, observed so the sidebar/pill react). No `GitStatusProviding` protocol — the app's `GitStatusService` sets the field directly; a single-implementer protocol with no agtCore consumer would be dead abstraction.
...
- [x] per-session throttle state lives on the `@MainActor` service — in-flight `Set<UUID>`, last-ran-cwd `[UUID:String]`, last-ran-at `[UUID:Date]`, read/written only on the main actor; the spawn/skip decision calls `agtCore` `GitRefreshPolicy.shouldRefresh(...)`. The worker never touches this state
...
- [x] `swift test` still green (agtCore unaffected — 75 tests pass)
...
- [x] `swift test` green; existing `agtUITests` (7) still pass
...
- [x] **window title: drive `NSWindow.title` directly — this is the primary path, not a fallback.** The scene is `Window("agt", id: "main")` (a fixed string literal owns the titlebar; verified `agtApp.swift:13`), so `.navigationTitle` on the `NavigationSplitView` detail does NOT reliably override it. Use a small `WindowAccessor` (`NSViewRepresentable` reading `view.window`, deferred via `DispatchQueue.main.async` since the window is nil at make time) and set `window.title = store.activeSession?.displayName ?? "agt"`, updated on change. (`.navigationTitle` may be set too, but it is not load-bearing.)
...
- [x] `swift test` green; `agtUITests` (7) still pass
...
- [x] full gates: `cd agtCore && swift test` green (75 tests pass); clean app build + `agtUITests` (7) were green at Task 6 and this docs-only change cannot regress them
- [x] update `README.md` (Features: git status in sidebar + title), `ARCHITECTURE.md` (the `agtCore` `GitStatus` + the app `GitStatusService` + refresh model), `CLAUDE.md` (git integration notes: the two git calls, the refresh/throttle model, agtCore-stays-git-free)
...
**Files:**
- Create: `agtUITests/GitStatusUITests.swift`
- Modify: `agtCore/Sources/agtCore/Session.swift` (add `effectiveCwd`)
- Modify: `agtCore/Tests/agtCoreTests/SessionTests.swift` (cover `effectiveCwd`)
- Modify: `agt/Git/GitStatusService.swift` (refresh against the effective cwd)

This turns the Post-Completion stretch into a required test: a real temp git repo driven into known states, the actual app launched against a seeded `workspaces.json`, and the sidebar `git-compact` token / title `git-pill` asserted against the real git state — the highest-value verification of the feature (nothing else exercised the integration end-to-end; the agtCore tests only feed canned strings to the parser).
...
- [x] seed persistence per test: hand-written `workspaces.json` matching the `Snapshot` Codable schema (`version`/`selectedSessionID`/`workspaces` → `id`/`name`/`sessions` → `id`/`cwd`), session `cwd` = temp repo (→ restored `initialCwd`), `selectedSessionID` set so the session is active on launch; `AGT_STATE_DIR` via `launchEnvironment`
...
- [x] gates green: `cd agtCore && swift test` (92 tests), clean Debug build (zero strict-concurrency warnings), full `agtUITests` — **11 tests, 0 skipped, 0 failures** (7 existing SidebarUITests + 4 new git tests); the git tests pass deterministically across repeated runs
