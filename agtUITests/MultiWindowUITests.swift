...
/// End-to-end tests for the multi-window scene: seed `AGT_STATE_DIR` with a `windows.json` index +
...
///
/// The seed is written as raw JSON (the UI-test target doesn't link `agtCore`) matching the
...
final class MultiWindowUITests: XCTestCase {
    ...
    override func setUp() async throws {
        ...
        stateDir = FileManager.default.temporaryDirectory
            .appendingPathComponent("agt-multiwin-\(UUID().uuidString)", isDirectory: true)
        try FileManager.default.createDirectory(at: stateDir, withIntermediateDirectories: true)
        socketPath = (NSTemporaryDirectory() as NSString)
            .appendingPathComponent("agtm-\(UUID().uuidString.prefix(8)).sock")
        markerDir = URL(fileURLWithPath: (NSTemporaryDirectory() as NSString)
            .appendingPathComponent("agtm-mark-\(UUID().uuidString.prefix(8))"), isDirectory: true)
    ...
    // into each window's own seeded session writes that window's $AGT_WINDOW_ID, and the two values must
    ...
    func testReopensTwoSeededWindows() throws {
        ...
        // read back the $AGT_WINDOW_ID its shell saw. Each must equal that window's id, and the two must
        ...
        XCTAssertEqual(aValue, windowAID.uuidString.lowercased(),
                       "window A's session should see AGT_WINDOW_ID == window A's id")
        XCTAssertEqual(bValue, windowBID.uuidString.lowercased(),
                       "window B's session should see AGT_WINDOW_ID == window B's id")
        XCTAssertNotEqual(aValue, bValue,
                          "the two windows must bind DISTINCT stores — identical ids would mean one shared store")
    }

    /// Type `echo "$AGT_WINDOW_ID" > <file>` into the given window's session (targeting it by id and
    ...
    private func readWindowEnv(windowID: UUID, sessionID: UUID, fileName: String) throws -> String {
        ...
        let cmd = "echo \"$AGT_WINDOW_ID\" > '\(file.path)'\n"
    ...
    func testClosingOneWindowRemovesIt() throws {
        ...
        // AGT_STATE_DIR) under the test's launch — the app's dedup mostly dismisses it, but it can sit
    ...
    // the spawned shell sees the AGT_* env: create a session over the control socket (so its surface
    // is realized after the socket bound, and AGT_SOCKET is populated), then type `echo "$AGT_WINDOW_ID"`
    // / `echo "$AGT_SESSION_ID"` into it and read the written files back (the split-test write-to-file
    // idiom). AGT_WINDOW_ID must equal the owning (frontmost) window's id and AGT_SESSION_ID the new
    ...
    func testSpawnedShellSeesWindowAndSessionEnv() throws {
        ...
        // (and was created after the control socket bound, so AGT_SOCKET is set).
        ...
        // echo $AGT_WINDOW_ID into a file and read it back: it must equal the frontmost window's id. The
        ...
        let windowCmd = "echo \"$AGT_WINDOW_ID\" > '\(windowFile.path)'\n"
        ...
        XCTAssertEqual(readWindowID.lowercased(), frontmost.lowercased(),
                       "AGT_WINDOW_ID should equal the owning (frontmost) window's id")

        // echo $AGT_SESSION_ID into a file and read it back: it must equal the new session's id.
        let sessionFile = markerDir.appendingPathComponent("session-id")
        let sessionCmd = "echo \"$AGT_SESSION_ID\" > '\(sessionFile.path)'\n"
        ...
        XCTAssertEqual(readSessionID.lowercased(), newSessionID.lowercased(),
                       "AGT_SESSION_ID should equal the spawning session's id")
    }

    // a session created in (and shown in) window B sees AGT_WINDOW_ID == window B's id, NOT the
    ...
    func testSessionEnvBindsToOwningWindowB() throws {
        ...
        // the B-session's shell must see AGT_WINDOW_ID == window B's id.
        ...
        XCTAssertEqual(readWindowID.lowercased(), windowBID.uuidString.lowercased(),
                       "AGT_WINDOW_ID should equal window B's id, not the frontmost-at-launch window A")
    }
