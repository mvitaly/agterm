...
struct SocketClientTests {
    ...
    @Test func connectFailureToMissingSocketThrows() {
        let client = SocketClient(path: NSTemporaryDirectory() + "agt-missing-\(UUID().uuidString.prefix(8)).sock")
...
private final class StubServer: @unchecked Sendable {
    ...
    init(response: ControlResponse) {
        ...
        self.path = NSTemporaryDirectory() + "agt-stub-\(UUID().uuidString.prefix(8)).sock"
    }
