...
/// A blocking, one-request-per-connection client for the agt control socket: connect to a unix domain
...
struct SocketClient {
    ...
    private func connect() throws -> Int32 {
        ...
        guard result == 0 else {
            ...
            close(fd)
            throw SocketClientError("connect(\(path)) failed: \(message) — is agt running?")
        }
        return fd
    }
